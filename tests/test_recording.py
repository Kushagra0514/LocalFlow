...
class HotkeyTest(unittest.TestCase):
    @staticmethod
    def recorder():
        return Recorder("ctrl+shift+space", MagicMock(return_value=True), MagicMock(), MagicMock())
    ...
    def test_single_key_repeat_is_ignored(self):
        recorder = Recorder("f23", MagicMock(return_value=True), MagicMock(), MagicMock())
        recorder.start = MagicMock()
        recorder.stop = MagicMock()
...
class RecorderTest(unittest.TestCase):
    ...
    def test_resources_and_purpose_are_owned_together(self):
        recorder = self.make_recorder(purpose=JobPurpose.DICTATION)
        recorder.start()
        recorder.purpose = JobPurpose.COMMAND
